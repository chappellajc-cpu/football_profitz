...
class FootballProfitZApp extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) => MaterialApp(title: 'Football ProfitZ', debugShowCheckedModeBanner: false, theme: ThemeData(colorScheme: ColorScheme.fromSeed(seedColor: Colors.green, brightness: Brightness.light)), darkTheme: ThemeData(colorScheme: ColorScheme.fromSeed(seedColor: Colors.green, brightness: Brightness.dark)), themeMode: ThemeMode.dark, home: const HomeScreen());
}
...
class _HomeScreenState extends State<HomeScreen> {
  ...
  final List<Map<String, dynamic>> _teams = [
    {'name': 'Man City', 'league': 'Premier League', 'played': 30, 'wins': 22, 'draws': 4, 'losses': 4, 'gf': 71, 'ga': 22, 'over25': 73.3, 'btts': 66.7, 'avgGF': 2.37},
    {'name': 'Liverpool', 'league': 'Premier League', 'played': 30, 'wins': 20, 'draws': 6, 'losses': 4, 'gf': 58, 'ga': 25, 'over25': 63.3, 'btts': 60.0, 'avgGF': 1.93},
    {'name': 'Arsenal', 'league': 'Premier League', 'played': 31, 'wins': 21, 'draws': 5, 'losses': 5, 'gf': 62, 'ga': 26, 'over25': 67.7, 'btts': 61.3, 'avgGF': 2.0},
    {'name': 'Bayern', 'league': 'Bundesliga', 'played': 28, 'wins': 22, 'draws': 3, 'losses': 3, 'gf': 79, 'ga': 28, 'over25': 75.0, 'btts': 71.4, 'avgGF': 2.82},
    {'name': 'Dortmund', 'league': 'Bundesliga', 'played': 28, 'wins': 17, 'draws': 5, 'losses': 6, 'gf': 60, 'ga': 36, 'over25': 67.9, 'btts': 64.3, 'avgGF': 2.14},
    {'name': 'Barcelona', 'league': 'La Liga', 'played': 29, 'wins': 20, 'draws': 5, 'losses': 4, 'gf': 59, 'ga': 26, 'over25': 65.5, 'btts': 58.6, 'avgGF': 2.03},
  ];
...
return Scaffold(
      appBar: AppBar(title: const Text('Football ProfitZ'), backgroundColor: Colors.green.shade800, foregroundColor: Colors.white),
  ...
  Widget _buildDashboard() {
    ...
    return ListView(padding: const EdgeInsets.all(16), children: [
      ...
      ...valueBets.map((vb) => Card(color: Colors.green.shade900.withOpacity(0.2), child: ListTile(leading: CircleAvatar(backgroundColor: Colors.green, child: Text(vb['label'], style: const TextStyle(fontSize: 12))), title: Text(vb['match']), subtitle: Text('${vb['market']} @ ${vb['odds']}'), trailing: Container(padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 6), decoration: BoxDecoration(color: Colors.green, borderRadius: BorderRadius.circular(20)), child: Text('+${vb['value']}%', style: const TextStyle(fontWeight: FontWeight.bold, color: Colors.white)))))),
      const SizedBox(height: 20),
...
Widget _buildLeagues() => ListView(padding: const EdgeInsets.all(16), children: [
    ...
    ..._teams.map((t) => Card(child: ExpansionTile(leading: CircleAvatar(backgroundColor: Colors.green.shade800, child: Text(t['name'][0], style: const TextStyle(color: Colors.white))), title: Text(t['name']), subtitle: Text(t['league']), children: [Padding(padding: const EdgeInsets.all(16), child: Column(children: [Row(children: [_s('Played', '${t['played']}'), _s('Wins', '${t['wins']}'), _s('Pts', '${t['wins'] * 3 + t['draws']}')]), const SizedBox(height: 8), Row(children: [_s('GF', '${t['gf']}'), _s('GA', '${t['ga']}'), _s('GD', '${t['gf'] - t['ga']}')]), const SizedBox(height: 8), Row(children: [_s('O2.5%', '${t['over25']}%'), _s('BTTS%', '${t['btts']}%')])]))])))),
  ]);
  Widget _s(String l, String v) => Expanded(child: Column(children: [Text(v, style: const TextStyle(fontWeight: FontWeight.bold)), Text(l, style: TextStyle(fontSize: 11, color: Colors.grey))]));
  ...
  Widget _buildValueBets() {
    ...
    return ListView(padding: const EdgeInsets.all(16), children: [
      ...
      if (bets.isEmpty) const Center(child: Padding(padding: EdgeInsets.all(32), child: Text('No value bets found')))
        ...
        const SizedBox(height: 8),
        Row(children: [_vbStat('Odds', '${vb['odds']}'), _vbStat('Calc', '${vb['calcProb']}%'), _vbStat('Implied', '${vb['implied']}%')]),
        const SizedBox(height: 12),
        Container(padding: const EdgeInsets.all(12), decoration: BoxDecoration(color: Colors.green.withOpacity(0.2), borderRadius: BorderRadius.circular(8)), child: Text(vb['rec'], style: const TextStyle(fontWeight:
 FontWeight.bold))),
      ])))),
    ]);
  }
  ...
  List<Map<String, dynamic>> _getValueBets() {
    ...
    for (final m in _matches.where((x) => x['status'] == 'Scheduled')) {
      ...
      if (home.isNotEmpty && m['over25'] != null) {
        ...
        if (value > 5) {
          bets.add({'match': '${m['home']} vs ${m['away']}', 'market': 'Over 2.5 Goals', 'odds': m['over25'], 'calcProb': calcProb.toStringAsFixed(1), 'implied': implied.toStringAsFixed(1), 'value': value.toStringAsFixed(1), 'label': value > 15 ? '🔥🔥🔥' : value > 10 ? '🔥🔥' : '🔥', 'rec': calcProb > 55 ? '✅ BACK' : '❌ LAY'});
        }
      }
    }
